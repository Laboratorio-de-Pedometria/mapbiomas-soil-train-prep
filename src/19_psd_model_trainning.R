...
file_name <- "psd_c02beta_000_010cm_v2.csv"
soildata_000_010cm <- data.table::fread(paste0(folder_path, file_name))
# Check the data
dim(soildata_000_010cm)
# Rows: 19944
# Columns: 79

# Drop unwanted covariates
colnames(soildata_000_010cm)
soildata_000_010cm <- soildata_000_010cm[
  ,
...
log_clay_sand_000_010cm_model <- ranger::ranger(
  formula = log_clay_sand ~ .,
  data = soildata_000_010cm[, !c("log_silt_sand")],
  num.trees = 100,
  mtry = 16,
  min.node.size = 2,
  max.depth = 30,
...
# Print model summary
print(log_clay_sand_000_010cm_model)
# Compute error metrics
observed <- soildata_000_010cm$log_clay_sand
predicted <- log_clay_sand_000_010cm_model$predictions
...
importance <- log_clay_sand_000_010cm_model$variable.importance
...
png("res/fig/psd_varimp_log_clay_sand_000_010cm.png", width = 480 * 3, height = 480 * 2, res = 72 * 2)
...
log_silt_sand_000_010cm_model <- ranger::ranger(
  formula = log_silt_sand ~ .,
  data = soildata_000_010cm[, !c("log_clay_sand")],
  num.trees = 100,
  mtry = 16,
  min.node.size = 2,
  max.depth = 30,
...
# Print model summary
print(log_silt_sand_000_010cm_model)
# Compute error metrics
observed <- soildata_000_010cm$log_silt_sand
predicted <- log_silt_sand_000_010cm_model$predictions
error <- observed - predicted
residual <- mean(observed) - observed
me <- mean(error)
mae <- mean(abs(error))
...
# Compute variable importance
importance <- log_silt_sand_000_010cm_model$variable.importance
importance <- data.table(
  variable = names(importance),
  importance = importance
)
...
num_plots <- ceiling(num_vars / vars_per_plot)
if (num_plots > 0) {
  png("res/fig/psd_varimp_log_silt_sand_000_010cm.png", width = 480 * 3, height = 480 * 2, res = 72 * 2)
  par(mar = c(5, 10, 2, 2), mfrow = c(1, num_plots))
  xlim <- c(0, max(importance$importance))
  for (i in 1:num_plots) {
    start_idx <- (i - 1) * vars_per_plot + 1
